    ...
    def forward(self, Xi, Xv):
        ...
        if self.use_fm:
            ...
            for i in range(len(Xi)):
                ...
                for i in range(self.field_size):
                    for j in range(i + 1, self.field_size):
                        fm_wij_arr.append(second_emb_list[i] * second_emb_list[j])
                fm_second_order_emb_arr.append(torch.cat(fm_wij_arr, 1))
        ...
        """
        if self.use_fm:
            interaction_layer = torch.cat(fm_second_order_emb_arr, 0)
        else:
            interaction_layer = torch.cat(fm_second_order_emb_arr, 0)
